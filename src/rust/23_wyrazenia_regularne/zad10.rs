...
fn podmien_slowa(tekst: &str, lista_a: &[String], lista_b: &[String]) -> String {
    ...
    for (a, b) in lista_a.iter().zip(lista_b.iter()) {
        ...
        let nowe_slowa: Vec<String> = slowa
            .iter()
            .map(|&slowo| {
                // Usuń znaki interpunkcyjne z końca
                let (koniec_inter, slowo_czyste) =
                    if let Some(ostatni_idx) = slowo.rfind(|c: char| c.is_alphanumeric()) {
...
fn main() {
    let stdin = io::stdin();
    let mut wiersze: Vec<String> = stdin.lock().lines().filter_map(|line| line.ok()).collect();
