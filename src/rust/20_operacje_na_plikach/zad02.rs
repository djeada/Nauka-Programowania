            ...
            if let Ok(file_type) = entry.file_type() {
                if file_type.is_file() {
                    if let Some(ext) = entry.path().extension() {
                        ...
                        let rozszerzenie_bez_kropki = rozszerzenie.trim_start_matches('.').to_lowercase();
...
fn main() {
    ...
    io::stdin().read_line(&mut folder_input).expect("Błąd wczytywania");
    io::stdin().read_line(&mut ext_input).expect("Błąd wczytywania");
