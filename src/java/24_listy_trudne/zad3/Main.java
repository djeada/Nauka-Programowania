...
public class Main {
   ...
   public static int minIloczynV1(ArrayList<Integer> lista) {
      ...
      if (n <= 2)
         throw new IllegalArgumentException("Lista musi miec co najmniej 3 elementy.");

      Collections.sort(lista);

      return Math.min(lista.get(n - 1) * lista.get(n - 2) * lista.get(0),
    ...
    public static int minIloczynV2(ArrayList<Integer> lista) {
        ...
        for (int i = 0; i < n - 2; i++)
            wynik = Math.min(wynik, Math.min(lista.get(i + 1) * minLewo.get(i + 1) * minPrawo.get(i + 1),
                                            lista.get(i + 1) * minLewo.get(i + 1) * maksPrawo.get(i + 1),
                                            lista.get(i + 1) * maksLewo.get(i + 1) * minPrawo.get(i +
