...
def zaczynaSieLubKonczyV1(tekst, napisA, napisB):
	return [e for e in tekst.splitlines() if re.search(r'^{0}'.format(napisA), e, flags=re.M) or re.search(r'{0}[!?.,;]$'.format(napisB), e, flags=re.M)]
...
Milosci pragną nie pragna tu zlota.""")
	...
	wynik = ['Folgujmy paniom nie sobie, ma rada;', 'Milujmy wiernie nie jest w nich przysada.', 'Milosci pragną nie pragna tu zlota.']
