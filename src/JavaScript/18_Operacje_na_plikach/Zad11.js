...
test1 = async () => {
    ...
    if (wynik1.length !== oczekiwane1.length) {
        throw new Error(`Assertion error line 34: ${wynik1.length} != ${oczekiwane1.length}`);
    }

    for (let i = 0; i < wynik1.length; i++) {
        if (wynik1[i] !== oczekiwane1[i]) {
            throw new Error(`Assertion error line 34: ${wynik1[i]} != ${oczekiwane1[i]}`);
        }
    }
    ...
    if (wynik2.length !== oczekiwane2.length) {
        throw new Error(`Assertion error line 34: ${wynik2.length} != ${oczekiwane2.length}`);
    }

    for (let i = 0; i < wynik2.length; i++) {
        if (wynik2[i] !== oczekiwane2[i]) {
            throw new Error(`Assertion error line 34: ${wynik2[i]} != ${oczekiwane2[i]}`);
        }
    }
