...
anagramy = function (napis) {
  ...
  for (const slowo of slowa) {
    for (const slowo2 of slowa) {
      if (
        slowo === slowo2 ||
        wynik.some((e) => e.first === slowo2 && e.second === slowo)
      ) {
        continue;
      }
...
test2 = function () {
  ...
  if (!(wynik.length == oczekiwane.length)) {
    throw new Error(
      `Assertion error line 54: ${wynik.length} != ${oczekiwane.length}`
    );
  }
  for (var i = 0; i < wynik.length; i++) {
    if (
      ...
      throw new Error(
        `Assertion error line 58: ${wynik[i].first} != ${oczekiwane[i].first} || ${wynik[i].second} != ${oczekiwane[i].second}`
      );
    }
  }
};
