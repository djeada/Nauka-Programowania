...
test5 = async () => {
    ...
    if (wynik.size !== oczekiwane.size) {
        throw new Error(`Assertion error line 180: ${wynik.size} != ${oczekiwane.size}`);
    }

    for (const [klucz, wartosc] of wynik) {
        if (oczekiwane.get(klucz) !== wartosc) {
            throw new Error(`Assertion error line 180: for key ${klucz}: ${wartosc} != ${oczekiwane.get(klucz)}`);
        }
    }
