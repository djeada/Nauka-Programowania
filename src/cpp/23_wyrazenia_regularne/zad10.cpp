...
Zastąp w tekście wszystkie wystąpienia słów z listy A odpowiadającymi im słowami
z listy B (ten sam indeks). Zamieniaj tylko **całe słowa**, nie fragmenty innych
słów.
...
void podmien(std::string &napis, std::vector<std::string> &listaA,
  ...
  if (listaA.size() != listaB.size()) return;
