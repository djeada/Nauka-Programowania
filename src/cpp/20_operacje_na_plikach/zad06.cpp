...
17.75
3.0
{'ala': 2, 'ma': 2, 'kota': 1, 'kot': 1, 'na': 1, 'imię': 1, 'filemon': 1,
'filemona': 1, 'lubi': 2, 'mleko': 1}
```

### Uwagi o formatowaniu

* Jeżeli ujednolicisz wielkość liter — w słowniku używaj małych liter (jak w
przykładzie).
...
int liczbaSlow(std::vector<std::string> &trescPliku) {
  ...
  for (std::string &wiersz : trescPliku) wynik += rozdzielSlowa(wiersz).size();
...
double sredniaDlugoscWiersza(std::vector<std::string> &trescPliku) {
  ...
  for (const std::string &wiersz : trescPliku) calkDlugosc += wiersz.size();
...
std::string wektorNaString(std::vector<std::string> &wektor) {
  std::string s;
  for (const auto &piece : wektor) s += piece;
  return s;
}
